...
func (t *TRH) Estimate(file string, labels []string, notes string) (satoshi.Satoshi, int, error) {
	...
	if err != nil {
		return 0, 0, fmt.Errorf("failed to generate entry from file: %w", err)
	}
	...
	if err != nil {
		return 0, 0, fmt.Errorf("failed to generate txs for entry: %w", err)
	}
	totFee := satoshi.Satoshi(0)
	for i, t := range txs {
		_, _, fee, err := t.TotInOutFee()
		if err != nil {
			return 0, 0, fmt.Errorf("failed to get fee from tx num %d: %w", i, err)
		}
