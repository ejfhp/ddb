...
func (b *Blockchain) Submit(txs []*DataTX) ([]string, error) {
	...
	log.Println(trace.Info("submit TX hex").UTC().Append(tr))
	ids := make([]string, len(txs))
	for i, tx := range txs {
		id, err := b.miner.SubmitTX(tx.ToString())
		if err != nil {
			log.Println(trace.Alert("cannot submit TX to miner").UTC().Add("i", fmt.Sprintf("%d", i)).Add("miner", b.miner.GetName()).Error(err).Append(tr))
			return nil, fmt.Errorf("cannot submit TX to miner: %W", err)
		}
		if id != tx.GetTxID() {
			log.Println(trace.Alert("miner returned a different TXID").UTC().Add("minerTXID", id).Add("TXID", tx.GetTxID()).Add("miner", b.miner.GetName()).Append(tr))
		}
		ids[i] = id
	}
	return ids, nil
