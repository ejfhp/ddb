...
func TestTransaction_NewTX(t *testing.T) {
	...
	datatx, err := ddb.NewTX(destinationKey, destinationAddress, changeAddress, utxos, amount, fee, payload)
	if err != nil {
	...
	if datatx.Outputs[0].Satoshis <= 0 {
		t.Logf("output num 0 should be the destination but has no output value: %d", datatx.Outputs[0].Satoshis)
		t.Fail()
	}
	if datatx.Outputs[2].Satoshis <= 0 {
	...
	input := datatx.Outputs[0].Satoshis + datatx.Outputs[2].Satoshis + uint64(fee.Satoshi())
	if input != uint64(bsv.Satoshi()) {
		t.Logf("Amounts don't match: %d + %d + %d != %d", datatx.Outputs[0].Satoshis, datatx.Outputs[2].Satoshis, uint64(fee.Satoshi()), input)
		t.Fail()
	}
}
func TestTransaction_BuildDataTX(t *testing.T) {
	trail.SetWriter(os.Stdout)
	txid := "e6706b900df5a46253b8788f691cbe1506c1e9b76766f1f9d6b3602e1458f055"
	scriptHex := "76a9142f353ff06fe8c4d558b9f58dce952948252e5df788ac"
	payload := []byte("ddb - Remind My... by ejfhp")
...
func TestTransaction_FeeOfHEXTX(t *testing.T) {
	trail.SetWriter(os.Stdout)
	he := ""
	tx, err := ddb.DataTXFromHex(he)
	if err != nil {
