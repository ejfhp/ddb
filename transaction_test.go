...
func TestUTXOToAddress(t *testing.T) {
	...
	utxos := []*spocs.UTXO{
		{
			...
			Value:  0.000402740,
			ScriptPubKey: &spocs.ScriptPubKey{
	...
	tx, err := spocs.UTXOsToAddress(utxos, destination, key, 170)
	if err != nil {
...
func TestSubmitRealTX(t *testing.T) {
	...
	pretx, err := spocs.UTXOsToAddress(utxo, toAddress, fromKey, 0)
	if err != nil {
		t.Fatalf("failed to build TX: %v", err)
	}
	fee, err := spocs.CalculateFee(pretx.ToBytes(), fees)
	if err != nil {
		t.Fatalf("failed to calculate fee: %v", err)
	}
	tx, err := spocs.UTXOsToAddress(utxo, toAddress, fromKey, fee)
	if err != nil {
		t.Fatalf("failed to build TX: %v", err)
	}
...
func TestRealSweep(t *testing.T) {
	...
	id, err := spocs.Sweep(fromKey, toAddress)
	if err != nil {
