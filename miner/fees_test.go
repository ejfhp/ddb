package miner_test

import (
	"testing"

	"github.com/ejfhp/ddb/miner"
	"github.com/ejfhp/ddb/satoshi"
)

func TestFee_CalculateFee(t *testing.T) {
	// trail.SetWriter(os.Stdout)
	tx := []byte(``)
	sat500 := satoshi.Satoshi(500)
	sat250 := satoshi.Satoshi(250)
	fees := miner.Fees{
		{
			FeeType: "standard",
			MiningFee: miner.FeeUnit{
				Satoshis: &sat500,
				Bytes:    1000,
			},
			RelayFee: miner.FeeUnit{
				Satoshis: &sat250,
				Bytes:    1000,
			},
		},
		{
			FeeType: "data",
			MiningFee: miner.FeeUnit{
				Satoshis: &sat500,
				Bytes:    1000,
			},
			RelayFee: miner.FeeUnit{
				Satoshis: &sat500,
				Bytes:    1000,
			},
		},
	}
	stdFee, err := fees.GetStandardFee()
	if err != nil {
		t.Fatalf("failed to calculate fee: %v", err)
	}
	expected := satoshi.Satoshi(341)
	//Fee calculation must be consistent
	for i := 0; i < 200; i++ {
		fee := stdFee.CalculateFee(len(tx))
		if fee != expected {
			t.Fatalf("%d - fee should be %d but is %d", i, expected, fee)
		}

	}
}

func TestFee_CalculateFee1035(t *testing.T) {
	// trail.SetWriter(os.Stdout)
	tx := make([]byte, 1035)
	sat500 := satoshi.Satoshi(500)
	sat250 := satoshi.Satoshi(250)
	fees := miner.Fees{
		{
			FeeType: "standard",
			MiningFee: miner.FeeUnit{
				Satoshis: &sat500,
				Bytes:    1000,
			},
			RelayFee: miner.FeeUnit{
				Satoshis: &sat250,
				Bytes:    1000,
			},
		},
		{
			FeeType: "data",
			MiningFee: miner.FeeUnit{
				Satoshis: &sat500,
				Bytes:    1000,
			},
			RelayFee: miner.FeeUnit{
				Satoshis: &sat500,
				Bytes:    1000,
			},
		},
	}
	stdFee, err := fees.GetStandardFee()
	if err != nil {
		t.Fatalf("failed to calculate fee: %v", err)
	}
	expected := satoshi.Satoshi(520)
	//Fee calculation must be consistent
	for i := 0; i < 200; i++ {
		fee := stdFee.CalculateFee(len(tx))
		if fee != expected {
			t.Fatalf("%d - fee should be %d but is %d", i, expected, fee)
		}

	}
}
