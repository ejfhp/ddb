package miner_test

import (
	"strings"
	"testing"

	"github.com/ejfhp/ddb/miner"
)

func TestTAAL_GetFees(t *testing.T) {
	// trail.SetWriter(os.Stdout)
	taal := miner.NewTAAL()
	fees, err := taal.GetFees()
	if err != nil {
		t.Fatalf("error: %v", err)
	}
	if len(fees) < 2 {
		t.Fatalf("Incomplete fees array: %v", fees)
	}
	for i, f := range fees {
		t.Logf("%d val: %s  %d\n", i, f.FeeType, f.MiningFee.Satoshis)
	}
}

func TestTAAL_SubmitTX(t *testing.T) {
	// trail.SetWriter(os.Stdout)
	txText := ``
	taal := miner.NewTAAL()
	txid, err := taal.SubmitTX(txText)
	if txid != "" {
		if strings.Contains(err.Error(), "failure") == false {
			t.Fatalf("Miner should reply with an failure: %v", err)
		}
	}
	//fmt.Printf("txid: '%s'\n", txid)
}
func TestTAAL_SubmitMultiTX(t *testing.T) {
	// trail.SetWriter(os.Stdout)
	txText := ``
	taal := miner.NewTAAL()
	restxs, err := taal.SubmitMultiTX([]string{txText})
	if err != nil {
		t.Logf("miner call failed: %v", err)
		t.FailNow()
	}
	if len(restxs) != 1 {
		t.Logf("unexpected result cardinality: %d", len(restxs))
	}
	for id, res := range restxs {
		t.Logf("%s: %s", id, res)
	}
}
